layui.define(['table', 'jquery', 'element','restClient'], function(exports) {
	"use strict";

	var MOD_NAME = 'message',
		$ = layui.jquery,
		restClient = layui.restClient,
	...
	message.prototype.render = function(opt) {
		...
		if (option.url != false) {
			var res = restClient.getSync(option.url)
	...
	function createHtml(option) {
		...
		$.each(option.data, function(i, item) {
			...
			$.each(item.children, function(i, note) {
				note.time = note.time.substr(0,16)
